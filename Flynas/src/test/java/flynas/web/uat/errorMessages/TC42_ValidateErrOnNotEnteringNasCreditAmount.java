...
public class TC42_ValidateErrOnNotEnteringNasCreditAmount extends BookingPageFlow {
		...
		try {
			...
			if(isElementDisplayedTemp(BookingPageLocators.MandatePrompt)){
				...
				if(ErrorMsg.contains(ErrorMessage)){
					Reporter.SuccessReport("Validating EorrorMessage on not entering Nas-Credit amount", "Successfully verified the error message :"+ ErrorMsg);
					driver.close();
				}else{
					Reporter.failureReport("Validating EorrorMessage on not entering Nas-Credit amount", "Error Message is not as expected");
					driver.close();
				}
				
			}
			else{
				Reporter.failureReport("Validating EorrorMessage on not entering Nas-Credit amount", "Error promt not found");
				driver.close();
			}			
			...
			e.printStackTrace();
			Reporter.failureReport("TC42_ValidateErrOnNotEnteringNasCreditAmount", "Failed");
			driver.close();
		}
	}
	...
	public Object[][] createdata1() {
	    return (Object[][]) new Object[][] { 
	    		...
	    		"",
	    		xls.getCellValue("ErrorMessage", "Value10"),
