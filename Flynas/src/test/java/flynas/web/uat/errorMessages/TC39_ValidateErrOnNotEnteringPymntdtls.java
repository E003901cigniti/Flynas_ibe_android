...
public class TC39_ValidateErrOnNotEnteringPymntdtls extends BookingPageFlow {
		...
		try {
			...
			if(isElementDisplayedTemp(BookingPageLocators.MandatePrompt)){
				...
				if(ErrorMsg.contains(ErrorMessage)){
					Reporter.SuccessReport("Validating EorrorMessage On not entering CC details", "Successfully verified the error message :"+ ErrorMsg);
					driver.close();
				}else{
					Reporter.failureReport("Validating EorrorMessage On not entering CC details", "Error Message is not as expected");
					driver.close();
				}
				
			}
			else{
				Reporter.failureReport("Validating EorrorMessage On not entering CC details", "Error promt not found");
				driver.close();
			}			
			...
			e.printStackTrace();
			Reporter.failureReport("TC39_ValidateErrOnNotEnteringPymntdtls", "Failed");
			driver.close();
		}
	}
