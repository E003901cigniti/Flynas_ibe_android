...
public class TC40_ValidateErrOnInvalidCCDetails extends BookingPageFlow{
	
	ExcelReader xls = new ExcelReader(configProps.getProperty("TestDataIBEUAT"),"Errors_On_PnrRetrieval");

	@Test(dataProvider = "testData",groups={"Chrome"})
	
	public  void ValidateErrOnInvalidCCDetails (String Username,String Password, String tripType,String origin, String destination,
		...
		try {
			...
			if(isElementDisplayedTemp(BookingPageLocators.ErrorMsg1)){
				...
				if(ErrorMsg.contains(ErrorMessage)){
					Reporter.SuccessReport("Validating EorrorMessage On entering Invalid CC details", "Successfully verified the error message :"+ ErrorMsg);
					driver.close();
				}else{
					Reporter.failureReport("Validating EorrorMessage On entering Invalid CC details", "Error Message is not as expected");
					driver.close();
				}
				
			}
			else{
				Reporter.failureReport("Validating EorrorMessage On entering Invalid CC details", "Error promt not found");
				driver.close();
			}			
			...
			e.printStackTrace();
			Reporter.failureReport("TC40_ValidateErrOnInvalidCCDetails", "Failed");
			driver.close();
		}
	}
	...
	public Object[][] createdata1() {
	    return (Object[][]) new Object[][] { 
	    		...
	    		"",
	    		xls.getCellValue("Payment Type", "Value8"),
	    		xls.getCellValue("Booking Class", "Value1"),
	    		xls.getCellValue("Select Seat", "Value1"),
	    		"",
	    		xls.getCellValue("ErrorMessage", "Value8"),
